...
import 'package:weatherbet/app/api/weather_api.dart';
...
class WeatherAPI {
  ...
  Future<double> getTemperature(double lat, double lon) async {
    String url = 'latitude=$lat&longitude=$lon&hourly=temperature_2m&forecast_days=1';
    if (settings.degrees == 'fahrenheit') {
      url += '&temperature_unit=fahrenheit';
    }
    ...
    try {
      ...
      return response.data['hourly']['temperature_2m'][0].toDouble();
    } on DioException catch (e) {
      if (kDebugMode) {
        print(e);
      }
